...
const greeting = (word) => {
  ...
  word.forEach(function(element) {
    grettingString.push(element.toUpperCase() + '!');
  });
...
const speaker = (words, callback) => {
  let speakerArray = [];
  // Solution code here...
  words.forEach(function(callback) {
    speakerArray.push(callback.toUpperCase() + '!');
  });
  return speakerArray;
};
...
const createList = (availableItems) => {
  let itemsArray = [];
  // Solution code here...
  availableItems.forEach(itemsArray); {
    itemsArray.push(availableItems);
  }
  return itemsArray;
};
